...
#    global bead_coord basis
    ...
    #set coord {}
    #for {set i 1} {$i <= $numatoms} {incr i} {
    #    lappend coord $coords($i)
    #}
    # setting p_ (pointers)
    #puts [expr $numatoms * 3]
    #puts "Prepare to set array"
    set p_coord [float_array [expr {$numatoms * 3}]]
    set p_force [float_array [expr {$numatoms * 3}]]
    #puts "Done setting array"
    set atm 1
    #puts "Done creating arrays"
    for {set i 1} {$i <= $numatoms} {incr i} {
...
#     puts $i
      ...
      #puts $p_force
      float_destroy $p_force
      float_destroy $p_coord
    }
